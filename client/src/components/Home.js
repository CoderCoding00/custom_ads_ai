...
//         const { product, feature, audience} = req.body;
//         const completion = await openai.createCompletion({
//           model: 'text-davinci-003',
//           prompt: generatePrompt(product, feature, audience),
//           temperature: 0.6,
//           max_tokens: 2048,
//         });
//         res.status(200).json({ result: completion.data.choices[0].text });
//       }
//       function generatePrompt(product, feature, audience) {
//         return `Generate a custom social media post for a ${feature} ${product} that is targeted towards ${audience}.`;
//       }
...
//     //     try{
//     //         const response = await getToPathname.post(url, {json:params, headers: headers}).json();
...
function generatePrompt(product, feature, audience) {
  return `Generate a custom social media post for a ${audience} ${product}  that is targeted towards ${feature}.\n`;
}
